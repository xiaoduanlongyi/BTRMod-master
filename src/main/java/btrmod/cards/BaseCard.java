...
import static btrmod.util.TextureLoader.getCardTextureString;


public abstract class BaseCard extends CustomCard {
    final private static Map<String, DynamicVariable> customVars = new HashMap<>();

    protected static String makeID(String name) { return BTRMod.makeID(name); }
...
//        initializeCharacterIcon();
    ...
    public void upgrade()
    {
        if (!upgraded)
        {
            ...
            if (this.upgradesDescription)
            {
                ...
                {
                    BTRMod.logger.error("Card " + cardID + " upgrades description and has null upgrade description.");
                }
                else
                {
     ...
     * 临时修改自定义变量的值（不改变基础值）
     */
    public void setCustomVarValue(String key, int value) {
        LocalVarInfo var = cardVariables.get(key);
        if (var != null) {
            var.value = value;
            var.forceModified = (value != var.base);
        }
    }
     ...
     * 重置自定义变量到基础值
     */
    public void resetCustomVar(String key) {
        LocalVarInfo var = cardVariables.get(key);
        if (var != null) {
            var.value = var.base;
